...
#define _XOPEN_SOURCE                                                                 \
  600 /* https://pubs.opengroup.org/onlinepubs/007904875/functions/xsh_chap02_02.html \
       */
...
#pragma warning( \
    disable      \
    : 4996) /* allows usage of strncpy, strcpy, strcat, sprintf, fopen */
#pragma warning( \
    disable      \
    : 4100) /* allows usage of strncpy, strcpy, strcat, sprintf, fopen */
#pragma warning( \
    disable      \
    : 4706) /* allows usage of strncpy, strcpy, strcat, sprintf, fopen */
#endif
...
static int powershellPresent(
    void) { /*only on vista and above (or installed on xp)*/
    ...
    unsigned char aoResultRGB[3])       /* { 0 , 0 , 0 } */
{
  ...
  aoResultRGB[0] = rValue(cc.rgbResult);
  aoResultRGB[1] = gValue(cc.rgbResult);
    ...
    char const* aDefaultPathAndOrFile) /* NULL or "" */
{
  ...
  strcat(lDialogString, "--backtitle \"");
  strcat(lDialogString,
         "tab: focus | /: populate | spacebar: fill text field | ok: TEXT "
         "FIELD ONLY");
  strcat(lDialogString, "\" ");
  ...
  for (i = 0; i < aNumOfFilterPatterns; i++) {
    if (tfd_quoteDetected(aFilterPatterns[i]))
      return tinyfd_saveFileDialog(
          "INVALID FILTER_PATTERN WITH QUOTES: use the GRAVE ACCENT \\x60 "
          "instead.",
...
static int perlPresent(void) {
  ...
  if (lPerlPresent < 0) {
    ...
    if (lPerlPresent) {
      lIn = (FILE*)popen(
          "perl -MNet::DBus -e "
          "\"Net::DBus->session->get_service('org.freedesktop.Notifications')"
          "\" 2>&1",
          "r");
...
static int afplayPresent(void) {
  ...
  if (lAfplayPresent < 0) {
    ...
    if (lAfplayPresent) {
      lIn = (FILE*)popen(
          "test -e /System/Library/Sounds/Ping.aiff || echo Ping", "r");
      if (fgets(lBuff, sizeof(lBuff), lIn) == NULL) {
        lAfplayPresent = 2;
      }
      pclose(lIn);
...
static int dunstPresent(void) {
  ...
  if (lDunstPresent < 0) {
    ...
    if (lDunstPresent) {
      lIn =
          (FILE*)popen("ps -e | grep dunst | grep -v grep",
...
int tfd_xpropPresent(void) {
  ...
  if (!lXpropReady) { /* xwayland Debian issue reported by Kay F. Jahnke and
                                ...
                                yesnocancel */
{
  ...
  if (osascriptPresent()) {
    ...
    strcat(lDialogString,
           " -e 'try' -e 'set {vButton} to {button returned} of ( display "
           "dialog \"");
    if (aMessage && strlen(aMessage)) {
      strcat(lDialogString, aMessage);
    }
...
-e 'else' -e 'return 0' -e 'end if' ");
    ...
    if (!strcmp("yesnocancel", aDialogType)) {
      strcat(lDialogString,
             "; x=$? ;if [ $x = 0 ] ;then echo 1;elif [ $x = 1 ] ;then echo "
             "2;else echo 0;fi");
    } else {
      strcat(lDialogString, ";if [ $? = 0 ];then echo 1;else echo 0;fi");
             ...
             tfd_shantyPresent() || tfd_boxerPresent()) {
    if (tfd_zenityPresent()) {
      if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
      ...
      if ((tfd_zenityVersion() >= 3010) && !getenv("SSH_TTY") &&
          tfd_xpropPresent()) {
        strcat(
            lDialogString,
    ...
    if (!strcmp("yesnocancel", aDialogType)) {
      strcat(lDialogString,
             ");if [ $? = 1 ];then echo 0;elif [ $szAnswer = \"No\" ];then "
             "echo 2;else echo 1;fi");
    } else {
      strcat(lDialogString, ");if [ $? = 0 ];then echo 1;else echo 0;fi");
    }
  }
...
'org.freedesktop.Notifications');");
    ...
    strcat(lDialogString, "\"");
  } else if (!isTerminalRunning() && notifyPresent() &&
             !strcmp("ok", aDialogType)) {
    if (aTitle && !strcmp(aTitle, "tinyfd_query")) {
...
to set mycolor to choose color default color {");
    ...
    strcat(lDialogString, "}' ");
    strcat(lDialogString,
           "-e 'set mystring to ((item 1 of mycolor) div 256 as integer) as "
           "string' ");
    strcat(lDialogString, "-e 'repeat with i from 2 to the count of mycolor' ");
    strcat(lDialogString,
           "-e 'set mystring to mystring & \" \" & ((item i of mycolor) div "
  ...
  } else if ((tfd_zenityVersion() >= 2032) || tfd_matedialogPresent() ||
             tfd_shellementaryPresent() || tfd_qarmaPresent() ||
             tfd_shantyPresent() || tfd_boxerPresent()) {
    lWasZenity2032 = 1;
    ...
    if (tfd_zenityVersion() >= 2032 || tfd_matedialogPresent() ||
        tfd_shellementaryPresent() || tfd_qarmaPresent() ||
        tfd_boxerPresent()) {
