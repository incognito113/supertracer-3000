    ...
    while (std::getline(file, currentLineString)) {
        ...
        if (currentLineString[0] == 'v') {
            ...
            while (std::getline(currentLine, vertexIndex, ' ')) {
                ...
                std::getline(vertexIndexStream, vertexIndex, '/'); // gets just the vertex from the block (we don't need anything else)
                faceVertices.push_back(vertexList[stoi(vertexIndex)]); // adds the indexed vertex the list of vertices for this face
            }
